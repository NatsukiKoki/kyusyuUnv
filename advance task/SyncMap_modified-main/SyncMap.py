...
class SyncMap:
	...
	def get_adaptation_rate(self,sequence_size,now_round):
		step=self.adaptation_rate/sequence_size
		step/=2
		if now_round<sequence_size//2:
			step=0
		return self.adaptation_rate-now_round*step
	...
	def inputGeneral(self, x):
		...
		for i in range(sequence_size):
			...
			self.syncmap+= self.get_adaptation_rate(sequence_size,i)*update
...
#		print("map",self.syncmap)
	...
	def organize(self):
		...
		self.labels= getCluster(self.syncmap, 3)
